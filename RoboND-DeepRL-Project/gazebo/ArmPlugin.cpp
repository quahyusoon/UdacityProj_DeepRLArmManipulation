...
#define INPUT_WIDTH   64  // Chg from 512
#define INPUT_HEIGHT  64  // Chg from 512
#define OPTIMIZER "RMSprop" // Chg from "None"
#define LEARNING_RATE 0.2f  // Chg from 0.0f 
#define REPLAY_MEMORY 10000
#define BATCH_SIZE 64      // Chg from 8 
#define USE_LSTM true      // Chg from false
#define LSTM_SIZE 512      // Chg from 32
...
#define REWARD_WIN  100.0f   // Chg from 0.0f
#define REWARD_LOSS -100.0f  // Chg from -0.0f
...
#define COLLISION_GRIPMID  "arm::gripper_middle::middle_collision" //Added
// Animation Steps
#define ANIMATION_STEPS 1000

// Set Debug Mode
#define DEBUG false    // Chg from false
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		if (collisionCheck)
		{
                        ...
                        if(DEBUG) printf("Collision with any part of the robot arm \n");
			//rewardHistory = REWARD_WIN;     //For objective 1
			//newReward  = true;		//For objective 1
			//endEpisode = true;		//For objective 1
			//return;                       /For objective 1
			
			/*
			/ For objective 2 - collision between gripper base and object
			/
			*/
                        const bool gripperbasecollisionCheck = (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0 
						              ||strcmp(contacts->contact(i).collision2().c_str(), COLLISION_GRIPMID) == 0);
                        if (gripperbasecollisionCheck)
		        {
